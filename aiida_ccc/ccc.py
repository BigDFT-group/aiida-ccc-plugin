...
_SLURM_SUBMITTED_REGEXP = re.compile(r'(.*:\s*)?([Gg]ranted job allocation|[Ss]ubmitted [Bb]atch [Ss]ession)\s+(?P<jobid>\d+)')
...
class CCCJobResource(NodeNumberJobResource):
...
class CCCScheduler(Scheduler):
    ...
    _job_resource_class = CCCJobResource
    ...
    def _get_submit_script_header(self, job_tmpl):
        ...
        if job_tmpl.job_name:
            ...
            lines.append(f'#MSUB -r {job_title}')

        if job_tmpl.import_sys_environment:
            lines.append('#MSUB -i')

        if job_tmpl.sched_output_path:
            lines.append(f'#MSUB -o {job_tmpl.sched_output_path}')
        ...
        if job_tmpl.sched_join_files:
            ...
            if job_tmpl.sched_error_path:
                lines.append(f'#MSUB -e {job_tmpl.sched_error_path}')
            else:
                # To avoid automatic join of files
                lines.append('#MSUB -e slurm-%j.err')

        if job_tmpl.queue_name:
            lines.append(f'#MSUB -q {job_tmpl.queue_name}')

        if job_tmpl.account:
            lines.append(f'#MSUB -A {job_tmpl.account}')

        if job_tmpl.qos:
            lines.append(f'#MSUB -QOS {job_tmpl.qos}')
        ...
        lines.append(f'#MSUB -N {job_tmpl.job_resource.num_machines}')
        if job_tmpl.job_resource.num_mpiprocs_per_machine:
            lines.append(f'#MSUB -n {job_tmpl.job_resource.num_mpiprocs_per_machine}')

        if job_tmpl.job_resource.num_cores_per_mpiproc:
            lines.append(f'#MSUB -c {job_tmpl.job_resource.num_cores_per_mpiproc}')
        ...
        if job_tmpl.max_wallclock_seconds is not None:
            ...
            # if days == 0:
            #     lines.append(f'#MSUB -T {hours:02d}:{minutes:02d}:{seconds:02d}')
            # else:
            #     lines.append(f'#MSUB -T {days:d}-{hours:02d}:{minutes:02d}:{seconds:02d}')
            lines.append(f'#MSUB -T {tot_secs}')
        ...
        if job_tmpl.max_memory_kb:
            ...
            lines.append(f'#MSUB -M {physical_memory_kb // 1024}')
    ...
    def _get_submit_command(self, submit_script):
        ...
        """
        submit_command = f'ccc_msub {submit_script}'
